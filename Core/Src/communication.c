...
extern struct UART_Parameter uart1;
extern struct UART_Parameter uart2;

//Choose the style of the USART
#define  DMA_Read_data
//#define  Interrupt_Read_data

#if defined Interrupt_Read_data
...
//	printf("length: %d\r\n", data_length);
    HAL_UART_Transmit(huart, uart1.receive_buff, data_length, 0x200); //测试用，发出接收到的数据
//	HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin);

    switch (uart1.receive_buff[0]) {
        case 0x31:
            globalVariable.motion_flag = forward;
            break;
        case 0x32:
            globalVariable.motion_flag = backward;
            break;
        case 0x33:
            globalVariable.motion_flag = left;
            break;
        case 0x34:
            globalVariable.motion_flag = right;
            break;
        case 0x35:
            globalVariable.motion_flag = stop;
            break;
        case 0x36:
            globalVariable.speed -= 10;
            break;
        case 0x37:
            globalVariable.speed += 10;
            break;
    ...
    //--------------------------------------------------
    memset(uart1.receive_buff, 0, data_length);                       //清零接收缓冲区
    data_length = 0;

    HAL_UART_Receive_DMA(huart, (uint8_t *) uart1.receive_buff, BUFFER_SIZE); //重新开启DMA传输，每次255字节数
}
...
//	HAL_UART_Transmit(&huart1, uart2.receive_buff, data_length, 0x200); //测试用，发出接收到的数据
    ...
    //添加处理接收数据的代码，接收数据存在receive_buff内
    if (uart2.receive_buff[0] == 0x55) {
        switch (uart2.receive_buff[23]) {
...
//				angle[2] = (receive_buff2[7] << 8 | receive_buff2[6]);
                ...
                memcpy(&sAngle, &uart2.receive_buff[24], 8);

//				PITCH = (float) stcAngle.Angle[0] / 32768 * 180;
//				ROLL = (float) stcAngle.Angle[1] / 32768 * 180;
//				HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin);
                Angle_int.PITCH = sAngle.Angle[0];
                Angle_int.ROLL = sAngle.Angle[1];
                Angle_int.YAW = sAngle.Angle[2];
    ...
    //--------------------------------------------------
    memset(uart2.receive_buff, 0, data_length);                       //清零接收缓冲区
    data_length = 0;

    HAL_UART_Receive_DMA(huart, (uint8_t *) uart2.receive_buff, 255); //重新开启DMA传输，每次255字节数
