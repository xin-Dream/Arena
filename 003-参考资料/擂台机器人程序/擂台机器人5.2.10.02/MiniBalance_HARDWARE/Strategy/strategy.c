...
{
	if(Len_Upback>2000)//如果尾部向上的传感器检测到边缘的木板，头部朝向台子
	{
		while(Len_Back>1700)//缓速向前移动直到距离围栏较远,留出转向空间
		{
		Set_Speed_Safe_Show(LOW,LOW);
		}
		Turn_to(180);
	...
	{
		while(Len_Back<2000)//缓速向后移动直到距离台子或围栏较近
		{
		Set_Speed_Safe_Show(-LOW,-LOW);
		}
		
		if(Len_Upback>2000)//如果尾部向上的传感器检测到围栏的木板，头部朝向台子
		{
			while(Len_Back>1500)//缓速向前移动直到距离围栏较远,留出转向空间
			{
			Set_Speed_Safe_Show(LOW,LOW);
			}
			Turn_to(180);
			while(Len_Front<2000)//缓速向前移动直到距离围栏较近，留出冲刺距离
			{
			Set_Speed_Safe_Show(LOW,LOW);
			}
			baizheng();//上台冲刺之前，保证车身正对台子
		...
		else
		{
			...
			{
				Set_Speed_Safe_Show(LOW,LOW);//缓速向前移动直到距离围栏较近，留出冲刺距离
			}
			baizheng();//上台冲刺之前，保证车身正对台子
			dengtai();
		}
...
{
	if(Len_Front_Left>Length_Short&&Len_Front_Right>Length_Short)        //如果正前方有敌人
	{
...
//		}
	...
	else if(Len_Front_Left >Length_Short&&Len_Front_Right<Length_Long)//如果左前方有敌人且右前方没有
	{
		Set_Speed_Safe_Show(-FOLLOW,FOLLOW);
//		Turn_to(20);
	}
	else if(Len_Front_Right >Length_Short&&Len_Front_Left<Length_Long)//如果右前方有敌人且左前方没有
	{
		Set_Speed_Safe_Show(FOLLOW,-FOLLOW);
//		Turn_to(-20);
	}
	else if(Len_Left>Length_Short)//如果左侧有敌人
	{
	    Counterlockwise(90);
		
	}
	else if(Len_Right>Length_Short)//如果右侧有敌人
	{
	    Clockwise(90);
	}
	else if(Len_Back>Length_Short)//如果后方有敌人
	{
		Clockwise(180);
//		Set_Speed(80,80);
//	    delay_ms(100);
...
{
	baizheng();
	if(Len_Front>1050&&Len_Back>1000)
	{
		Front_Back();
	}
	
	else if(Len_Left>1100&&Len_Right>900)
	{
		Turn_to(90);
		Front_Back();
	}
