...
{  							  
  u8 temp,t1,t;
	u16 x0=x;
	u8 csize=(size/8+((size%8)?1:0))*size;//得到字体一个汉字对应点阵集所占的字节数	
//num=num-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	for(t=0;t<csize;t++)
	{
		if(size==12)temp=word_1212[num][t]; 	 	//调用1206字体
		else if(size==16)temp=word_1616[num][t];	//调用1608字体
		else if(size==24)temp=word_2424[num][t];	//调用2412字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
//x,y:起始坐标
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_Showpic_1(u16 x,u16 y)
{  							  
  u16 temp,t1,t;
	u16 x0=x;
	u16 y0=y;
