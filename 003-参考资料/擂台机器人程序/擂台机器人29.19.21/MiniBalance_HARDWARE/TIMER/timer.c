...
void TIM3_Config(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = 20000-1;			        //自动重装值
...
void TIM3_IRQHandler(void)
{
    ...
    { 
		...
		{
			time=0;//清除时间计数
//			if(NumGrayLess(850)>1&&NumLenMore(CM40)>0)//当有两个底部灰度传感器检测到灰度较大时
			if(NumGrayLess(1050)>1) //灰度据光线调整，光线调为1200
			{
				if(Gray_Left<900&&Gray_Mid<900&&Gray_Right<900&&NumLenMore(700)>=2)
				{
					dengtai_auto();
				}
				else
