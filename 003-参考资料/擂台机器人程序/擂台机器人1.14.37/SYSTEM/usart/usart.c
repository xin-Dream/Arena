...
//			if(Len_Edge_Left<2800||Len_Edge_Right<2800)
//			{
//			   Avoid_Falling_180();
//				
//			}
...
//	}
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) == SET)
	{
		...
		WT_buf[0] =USART_ReceiveData(USART3);	//读取接收到的数据					
		if(WT_buf[10]==0x55&&WT_buf[9]==0x53)
		{
			YAW=(WT_buf[3]<<8|WT_buf[4]);
			ROLL=(WT_buf[7]<<8|WT_buf[8]);
			PITCH=(WT_buf[5]<<8|WT_buf[6]);
...
{
	USART_SendData(USART3,0xff);delay_us(100);
	USART_SendData(USART3,0xaa);delay_us(100);
	USART_SendData(USART3,0x27);delay_us(100);
	USART_SendData(USART3,0xff);delay_us(100);
	USART_SendData(USART3,0x00);delay_us(100);

	USART_SendData(USART3,0xff);delay_us(100);
	USART_SendData(USART3,0xaa);delay_us(100);
	USART_SendData(USART3,0x76);delay_us(100);
	USART_SendData(USART3,0x00);delay_us(100);
	USART_SendData(USART3,0x00);delay_us(100);
}