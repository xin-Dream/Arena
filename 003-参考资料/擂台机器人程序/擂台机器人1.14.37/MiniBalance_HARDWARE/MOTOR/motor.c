...
void Turn_to(int pianzhuan)
{
	...
	while(1)
	{
		if(myabs(bias)<4000)
		{
			bias=Get_Bias(YAW,target);
			sum+=bias;
			Set_Speed_Show(-bias*parap-sum*parai,bias*parap+sum*parai);
			delay_ms_for(2);
		}
		else
		{
...
//	target=Get_Target(YAW,pianzhuan);
	while(1)
	{
		...
		else if(myabs(bias)<9000&&myabs(bias)>1000)
		{
			bias=Get_Bias(YAW,target);
			Set_Speed_Show(-bias*parap,bias*parap);
			delay_ms_for(2);
		...
		{
			bias=Get_Bias(YAW,target);
			Set_Speed_Show(-bias*parap1,bias*parap1);
			delay_ms_for(2);	
		}
		if(myabs(bias)<200)
		{
...
{
	if((0<YAW&&YAW<4500)|(36000>YAW&&YAW>31500))
	{
		Turn_to_abs(0);
	}
